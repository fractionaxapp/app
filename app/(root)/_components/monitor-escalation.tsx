...
import { Arrow, SectionBar } from "./ui";
...
export function MonitorEscalation() {
	return (
		<section className="fx-section fx-bleed">
			<SectionBar
				label="When it breaches"
				copy="Watching a number fall is only worth anything if something happens when it lands."
			/>
					...
					<ol className="px-5 py-2">
						{steps.map((step, index) => (
							<li key={step.label} className="flex gap-4 py-4">
								...
								{/* Three tracks: what happens, what it means, when. */}
								<div className="grid flex-1 gap-x-8 gap-y-1.5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.6fr)_7rem] lg:items-baseline">
									...
									<p className="fx-eyebrow text-muted lg:text-right">
										{step.at}
									</p>
								</div>
							</li>
